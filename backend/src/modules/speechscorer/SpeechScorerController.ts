...
export default class SpeechScorerController {
  ...
  @HttpCode(200)
  async analyzeSpeech(
    @Body() body: AnalyzeSpeechBody,
    @Res() res: Response,
  ) {
    try {
      const { transcript, durationInSeconds } = body;
      ...
      if (!durationInSeconds || durationInSeconds <= 0) {
        return res.status(400).json({
          message: 'durationInSeconds is required and must be a positive number.',
        });
      }

      const result = this.speechAnalysisService.analyze(transcript, durationInSeconds);
